...
def load_api_key() -> str:
    """Load YouTube API key from environment variable or credentials.yml file."""
    # Priority 1: Environment Variable
    env_key = os.getenv("YOUTUBE_API_KEY")
    if env_key:
        return env_key

    # Priority 2: credentials.yml
    ...
    try:
        ...
        # If file exists but key not found, that's an error only if env var was also missing
        raise ValueError("youtube_api_key not found in credentials.yml")
    except FileNotFoundError:
        # Expected if running in container without file mount, but we already checked env var
        raise ValueError("YOUTUBE_API_KEY environment variable not set and credentials.yml not found.")
...
# Expose the ASGI app for deployment (e.g. with Uvicorn)
# This allows: uvicorn youtube_mcp_server:app --host 0.0.0.0 --port 8000
app = mcp.sse_app
